...
const FACE_INK = ["#FAF4E8", "#2E2A24", "#FAF4E8", "#2E2A24", "#FAF4E8"];
...
export default function KidQDesktop() {
  ...
  if (stage === "profile") return <Shell label="Who's watching today?" bare><section className={styles.profileScreen}><div className={styles.profileStars} /><h1>Who&apos;s watching<br />today?</h1>{children.length > 0 ? <div className={styles.profileChoices}>{children.map((item, index) => <button key={item.id} onClick={() => chooseChild(item)}><span className={styles.faceWrap} style={{ width: 72, height: 72 }}><span className={styles.faceHalo} style={{ animationDelay: `${-(index % 2) * 2.2}s` }} /><AvatarFace disc={CHILD_COLOURS[index % CHILD_COLOURS.length]} face={FACE_INK[index % FACE_INK.length]} /></span><b>{item.nickname}</b><small>Start my day</small></button>)}</div> : childrenLoaded && <div className={styles.noProfiles}><p>No child profiles yet.</p><Link href="/parent">Set up a child profile in Parent view</Link></div>}<p className={styles.noLogin}>No child login needed — a parent sets up the profile.</p>{children[0] && <button className={styles.demoLink} onClick={() => chooseChild(children[0], true)}>Show no-session state</button>}</section></Shell>;
...
function AvatarFace({ disc, face }: { disc: string; face: string }) {
  return <svg viewBox="0 0 64 64" aria-hidden="true">
    <circle cx="32" cy="32" r="30" fill={disc} />
    <circle cx="25.5" cy="29" r="2.4" fill={face} />
    <circle cx="38.5" cy="29" r="2.4" fill={face} />
    <path d="M24.5 37 Q32 43 39.5 37" fill="none" stroke={face} strokeWidth="2.6" strokeLinecap="round" />
  </svg>;
}
